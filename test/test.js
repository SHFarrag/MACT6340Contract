...
describe("SHFCreativeNFTContract", async function () {
  let SHFCreativeNFTContractFactory;
  let SHFCreativeNFTContract;
  let args = {
  ...
  this.beforeEach(async function () {
    SHFCreativeNFTContractFactory = await ethers.getContractFactory(
      "SHFCreativeNFTContract"
    );
    SHFCreativeNFTContract = await SHFCreativeNFTContractFactory.deploy(
      args.mint_price,
      args.max_tokens,
      args.base_uri,
      args.royaltyArtist,
    ...
    await SHFCreativeNFTContract.waitForDeployment(
      args.mint_price,
      args.max_tokens,
      args.base_uri,
      args.royaltyArtist,
  ...
  describe("construction and initialization", async function () {
    ...
    it("should be named SHFCreativeNFTContract", async function () {
      const expectedValue = "SHFCreativeNFTContract";
      const currentValue = await SHFCreativeNFTContract.name();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should be have symbol DGS", async function () {
      const expectedValue = "DGS";
      const currentValue = await SHFCreativeNFTContract.symbol();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should have a mint price set when constructed", async function () {
      const expectedValue = args.mint_price;
      const currentValue = await SHFCreativeNFTContract.getMintPrice();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should have a max token supply set when constructed", async function () {
      const expectedValue = args.max_tokens;
      const currentValue = await SHFCreativeNFTContract.getMaxSupply();
      assert.equal(currentValue.toString(), expectedValue);
    });

    it("should have a base URI set when constructed", async function () {
      const expectedValue = args.base_uri;
      const currentValue = await SHFCreativeNFTContract.getBaseURI();
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should set roylaty artist when constructed", async function () {
      ...
      const currentValue = await SHFCreativeNFTContract.royaltyInfo(
        1,
    ...
    it("should set owner to the deployer's address when constucted", async function () {
      ...
      const currentValue = await SHFCreativeNFTContract.owner();
      assert.equal(currentValue.toString(), expectedValue);
    });
  });
  ...
  describe("receive function", async function () {
    ...
    it("should be called and revert if called from low-level transaction", async function () {
      let contractAddress = await SHFCreativeNFTContract.getAddress();
  ...
  describe("mintTo function", async function () {
    ...
    it("should revert if called with no ether", async function () {
      const [owner, artist, buyer] = await ethers.getSigners();
      expect(
        SHFCreativeNFTContract.connect(buyer).mintTo(args.base_uri, {
    ...
    it("should have a totalSupply = 1 with after first mint", async function () {
      ...
      const mint1 = await SHFCreativeNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint1.wait(1);
      const currentValue = await SHFCreativeNFTContract.totalSupply();
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should increase the totalSupply by 1 with with each mint", async function () {
      ...
      mint1.wait(1);
      const mint2 = await SHFCreativeNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint2.wait(1);
    ...
    it("should emit an event when minting is completed", async function () {
      ...
      expect(
        ...
        .to.emit(SHFCreativeNFTContract, "MintingCompleted")
        .withArgs(1, owner);
    });
    ...
    it("should have set the token uri during minting function", async function () {
      ...
      mint1.wait(1);
      const currentValue = await SHFCreativeNFTContract.tokenURI(1);
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should emit an event after funds are distributed during mint", async function () {
      ...
      const uriString = "someString";
      expect(
        SHFCreativeNFTContract.connect(buyer).mintTo(uriString, {
          value: args.mint_price,
        })
      )
        .to.emit(SHFCreativeNFTContract, "FundsDistributed")
