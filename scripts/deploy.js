...
async function main() {
  ...
  const SHFCreativeNFTContractFactory = await ethers.getContractFactory(
    "SHFCreativeNFTContract"
  );
  // deploy
  const SHFCreativeNFTContract = await SHFCreativeNFTContractFactory.deploy(
    args.mint_price,
    args.max_tokens,
    args.base_uri,
    args.royaltyArtist,
  ...
  await SHFCreativeNFTContract.deploymentTransaction().wait(15);
  ...
  for (let i = 0; i < 3; i++) {
    const transactionResponse = await SHFCreativeNFTContract.mintTo(ipfs[i], {
