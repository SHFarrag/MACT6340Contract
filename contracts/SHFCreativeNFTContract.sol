...
error SHFCreativeNFTContract__MaxSupplyReached();
error SHFCreativeNFTContract__ValueNotEqualPrice();
error SHFCreativeNFTContract__WrongAvenueForThisTransaction();

/// @custom:security-contact sfarrag@smu.edu
contract SHFCreativeNFTContract is 
    ERC721, 
    ERC721Enumerable, 
    ERC721URIStorage, 
    ERC721Burnable, 
    ...
    constructor(
        ...
        ERC721("SHFCreativeNFTContract", "SHF")
        Ownable(msg.sender)
    ...
    receive() external payable {
        revert SHFCreativeNFTContract__WrongAvenueForThisTransaction();
    }
    ...
    function mintTo(
        ...
        if (tokenId >= i_max_tokens) {
            revert SHFCreativeNFTContract__MaxSupplyReached();
        }
        // makes sure there is the right amount of money
        if (msg.value != i_mint_price) {
            revert SHFCreativeNFTContract__ValueNotEqualPrice();
        }
        _tokenIdCounter++;
